...
final class AIOrchestrator {
    ...
    private static func referenceInstruction(for mode: SourceContextMode) -> String {
        ...
        case .retrieved:
            ...
            When a passage genuinely supports a claim, cite its manifest entry inline as 【n|"exact quote"】 immediately after that claim, where the quote is a short verbatim excerpt of 5-20 words copied character-for-character from the cited passage that directly supports the claim. Use plain 【n】 only when no single short span applies. Cite a passage where it first supports the answer rather than repeating the same citation for every subsequent claim. Use at most two citations per claim. Use no other citation formats, footnotes, bracketed numbers, markdown links, or URLs. You may also answer from your own knowledge without a citation when the retrieved passages do not support that part of the answer.
            """
        }
    }
}
