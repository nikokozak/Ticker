...
## Driving the GUI
- Quick Panel: global hotkey ⌘L → `osascript`: activate "TickerNextDebug" (dev) / "TickerNext" (prod), `keystroke "l" using command down`, then `keystroke "<text>"`, `key code 36` (↵). ⌘↵ = `key code 36 using command down`; ⌥↵ = `using option down`.
