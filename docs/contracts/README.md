...
- `bridge.v2.json`: canonical live bidirectional bridge payload contract for the CodeMirror stream-document editor.
- `ticker-proxy.openapi.v1.yaml`: proxy API contract.

## Direction

Per `docs/TICKER_NEXT_MVP_PLAN.md`, Ticker-Next uses a no-cell stream-document editor model.

Implication:
- `bridge.v1.json` is historical and must not be used for live validation.
- New editor work should define stream-document oriented Swift-to-Web and Web-to-Swift bridge messages in `bridge.v2.json`.
