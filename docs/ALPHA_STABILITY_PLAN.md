...
Current state:
Status (2026-02-05): **Completed**
- The “think” request is **one** streamed proxy call (`includeHeading: true`).
- No `restatementGenerated` bridge message exists (bridge contract + Swift + Web).
- UI derives titles from the **editable heading in content**, not from `Cell.restatement`.
...
Ensure the following do **not** exist in the “think” flow:
- Any separate proxy call to compute a restatement (`generateRestatement(...)`)
- Any `restatementGenerated` bridge emission
- Any persistence writes to `cells.restatement`
...
There should be **no** restatement-specific field or bridge path:
- Web does not store a `restatement` field on `Cell` models.
- Web derives titles from the first heading in the rendered HTML content.
- There is no `restatementGenerated` message type in `SWIFT_TO_WEB_MESSAGE_TYPES`.
...
Implementation note:
- We implement **A4.2** as a DB migration (`v9_heading_in_content_titles`) that prepends a `<h2>` heading into `cells.content`
  for legacy rows that have `cells.restatement` but no heading in content.
